...
- Hosted-CI deprecation handling: GitHub forced the initially pinned checkout/setup/artifact/cache actions from deprecated Node 20 onto Node 24. The final candidate instead pins each official action's current native Node 24 major by exact commit; the FOSSA action is pinned to its current Node 24 main commit while its CLI remains fixed separately.
